Sprints.ready(function()
{
    var Task = function (el, panel)
    {
        ...
        // done ratio
        $('.task_doneratio_value', el).editable('/tasks/inplace', $.extend({name: 'done_ratio', type: 'ptext', width: 50, callback: function (res, settings)
        {
            res = +res;
        ...
        // spent
        $('.task_add_spent_value', el).editable('/tasks/spent', $.extend({name: 'hours', type: 'text', placeholder: 'X', width: 50, event: 'addspent', callback: function (res, settings)
        {
            $(this).html('X');
    ...
    {
        location.href = '/sprints_mine/list?project_id=' + Sprints.getProjectId() + '&sprint=' + $(this).val() + '&user=' +  $('#user_selection select').val();
    });
    $('#user_selection select').change(function ()
    {
        location.href = '/sprints_mine/list?project_id=' + Sprints.getProjectId() + '&sprint=' + $('#sprints_selection select').val() + '&user=' +  $(this).val();
    });
